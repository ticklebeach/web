...
float sdfModCube(vec3 _p, vec3 size, float round, float hole, float bevel, float cone) {
  ...
  if (hole > 0.) {
    if (bevel > 0.) {
      ...
      _d = opExtrussion(p1, _d, size.z - round);
    } else {
      float r2 = 1.;
...
float sdfStroke(int prim, vec3 _p, vec3 size, int mode, float round, float hole, float bevel, float cone, float lineWidth, float torus) {
  // return sdEllipsoid(_p, size);
  if (prim == 1) { // cube
    return sdfModCube(_p, size, round, hole, bevel, cone);
  } else if (prim == 2) { // cylinder
    return sdfModCylinder(_p, size, hole, cone, round);
  } else if (prim == 3) { // bezier
    return sdfModBezier(_p, size, mode, round, lineWidth, torus);
  } else if (prim == 4) {
    return sdfModJoint(_p, size, cone, round);
  } else if (prim == 5) {
    return sdEllipsoid(_p, size);
  } else {
    return sdEllipsoid(_p, size);
...
Result evalScene(vec3 p) {
  ...
  int i = 1;
  while (true) {
    ivec2 texPos = texturePos(i, stride);
    ...
    _p = rot * _p;
    _d = sdfStroke(prim, _p, size, mode, round, hole, bevel, cone, lineWidth, torus);
...
{
  return min(pos.y + 1.0, evalScene(pos).d);
}
...
void main() {
  // int treeIndex = gl_InstanceID / 8;
  // int octant = gl_InstanceID & 7;

  int treeIndex = int(iNodeIndex / 8.);
  int octant = int(iNodeIndex) & 7;

  ivec2 texPos = texturePos(treeIndex, 1);
  vec3 center = texelFetch(uTreeTex, texPos, 0).xyz;
  vec3 halfSize;
  if (uDoubleMaxDepth == 1) {
    halfSize = uBoundsSize / float(1 << (uDepth + 1 + (uDepth == uMaxDepth ? 0 : 1)));
  } else {
    halfSize = uBoundsSize / float(1 << (uDepth + 1 + 1));
  }

  int i = octant;
  vec3 pos = vec3(bvec3(((i & 1) > 0), ((i & 2) > 0), ((i & 4) > 0)));
  vec3 offs = halfSize * (pos * 2. - 1.);
  vec3 coord = center + offs;

  // sample sdf at center of octant
  Result res = evalScene(coord);

  oResult = vec4(res.d, res.m1, res.m2, res.b);
  oCoord = vec4(coord, 0.);

  float distToCorner = length(halfSize);

  // if distance = 0 could be within this octant
  if (abs(res.d) < distToCorner) {
    // if we aren't at the max depth
    if (uDepth < uMaxDepth) {
      // subdivide and recurse
      oNode.x = 1.; // type
    } else {
      // we are at a leaf node; record the distance for this node
      oNode.x = 0.; // type
      vec3 normal = GetNormal(coord);
      oNormal = vec4(normal, calcAO(coord, normal));
    }
  } else {
    oNode.x = -1.; // type
    if (uDoubleMaxDepth == 1 && uDepth == uMaxDepth) {
      vec3 normal = GetNormal(coord);
      oNormal = vec4(normal, calcAO(coord, normal));
    }
  }
